...
[Experience](#experience) View certificate · purple sector →
...
[Projects](#projects) View on GitHub · overtake →

## [Projects] Stormline — Real-Time Severe Weather Alerting Platform
...
[Projects](#projects) Most weather apps make the client do all the work. The phone keeps asking the server "anything new?" every few minutes, which is slow, wasteful and doesn't scale once thousands of clients are polling independently. The brief I set myself was to invert that so the server knows the moment conditions turn severe and pushes that update to every subscribed client instantly, with nobody having to ask — and to keep that guarantee even if the client was offline when it happened.
[Projects](#projects) Replaced client-side polling with server-pushed alerts: a dedicated poller container queries the free Open-Meteo API for each subscribed location once per cycle and publishes classified conditions over Redis Pub/Sub, rounding coordinates to ~1.1km so nearby subscriptions share a single polled point.
[Projects](#projects) Scaled live delivery horizontally across a three-channel Redis Pub/Sub design: each API replica forwards alerts only to its own locally-connected clients, while a second channel propagates subscription changes to an already-open socket on a *different* replica with no reconnect needed, and a third delivers per-user personalized alerts for subscribers running custom thresholds.
[Projects](#projects) Designed a severity engine with hand-tunable thresholds across four condition categories at three escalating tiers, with per-subscription overrides — so two people watching the same city can have genuinely different alert sensitivity for the same reading.
[Projects](#projects) Added JWT-based auth (short-lived access tokens, Postgres-tracked refresh tokens, Redis-backed revocation blacklist) and server-side missed-alert catch-up, so a client that reconnects — or opens the UI fresh — recovers everything it missed while offline, plus per-user alert dismissal that persists across devices.
[Projects](#projects) Wrote a 69-test pytest suite covering severity classification, connection-manager filtering, geocoding, the two-replica pub/sub fan-out, auth flows and an end-to-end WebSocket test — running entirely against SQLite and fakeredis, no live Postgres/Redis needed.
